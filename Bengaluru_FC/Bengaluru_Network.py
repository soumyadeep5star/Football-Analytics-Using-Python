...
def iterate(df_1):
    ...
    for i,j in df_1.iterrows(): #Here 'i' returns index and 'j' returns a series
        ...
        sub = df.loc[df["type_name"] == "Substitution"].loc[df["team_name"] == "Bengaluru"].iloc[0]["index"]
        #make df with successfull passes by Bengaluru until the first substitution
        mask_Bengaluru = (df.type_name == 'Pass') & (df.team_name == "Bengaluru") & (df.index < sub) & (df.outcome_name.isnull()) & (df.sub_type_name != "Throw-in")
        #taking necessary columns
        df_pass = df.loc[mask_Bengaluru, ['x', 'y', 'end_x', 'end_y', "player_name", "pass_recipient_name"]]
        ...
        fig.suptitle(f"Bengaluru Passing network against {opposition}.", fontsize = 30)
        #plt.show()

        #plot that is generated is stored in same directory where Team_Name_Script.py and Team_Name_Network.py is present.
        plt.savefig(f"Bengaluru FC\Bengaluru_draw_img/Fig_{count}")
