...
def iterate(df_1):
    ...
    for i,j in df_1.iterrows(): #Here 'i' returns index and 'j' returns a series
        ...
        sub = df.loc[df["type_name"] == "Substitution"].loc[df["team_name"] == "ATK Mohun Bagan"].iloc[0]["index"]
        #make df with successfull passes by ATK_Mohun_Bagan until the first substitution
        mask_ATK_Mohun_Bagan = (df.type_name == 'Pass') & (df.team_name == "ATK Mohun Bagan") & (df.index < sub) & (df.outcome_name.isnull()) & (df.sub_type_name != "Throw-in")
        #taking necessary columns
        df_pass = df.loc[mask_ATK_Mohun_Bagan, ['x', 'y', 'end_x', 'end_y', "player_name", "pass_recipient_name"]]
        ...
        fig.suptitle(f"ATK_Mohun_Bagan Passing network against {opposition}.", fontsize = 30)
        #plt.show()

        #plot that is generated is stored in same directory where Team_Name_Script.py and Team_Name_Network.py is present.
        plt.savefig(f"ATK Mohun Bagan_FC\ATK Mohun Bagan_win_img/Fig_{count}")
    ...
    #Calculating average Centrality Index
