...
def iterate(df_1):
    ...
    for i,j in df_1.iterrows(): #Here 'i' returns index and 'j' returns a series
        ...
        sub = df.loc[df["type_name"] == "Substitution"].loc[df["team_name"] == "NorthEast United"].iloc[0]["index"]
        #make df with successfull passes by NorthEast United until the first substitution
        mask_NE_United = (df.type_name == 'Pass') & (df.team_name == "NorthEast United") & (df.index < sub) & (df.outcome_name.isnull()) & (df.sub_type_name != "Throw-in")
        #taking necessary columns
        df_pass = df.loc[mask_NE_United, ['x', 'y', 'end_x', 'end_y', "player_name", "pass_recipient_name"]]
        ...
        fig.suptitle(f"NE_United Passing network against {opposition}.", fontsize = 30)
        #plt.show()
        plt.savefig(f"NorthEast United FC\I_NE_United_draw_img/Fig_{count}")
