...
def iterate(df_1):
    ...
    for i,j in df_1.iterrows(): #Here 'i' returns index and 'j' returns a series
        ...
        sub = df.loc[df["type_name"] == "Substitution"].loc[df["team_name"] == "Goa"].iloc[0]["index"]
        #make df with successfull passes by Goa until the first substitution
        mask_Goa = (df.type_name == 'Pass') & (df.team_name == "Goa") & (df.index < sub) & (df.outcome_name.isnull()) & (df.sub_type_name != "Throw-in")
        #taking necessary columns
        df_pass = df.loc[mask_Goa, ['x', 'y', 'end_x', 'end_y', "player_name", "pass_recipient_name"]]
